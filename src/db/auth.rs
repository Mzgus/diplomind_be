...
pub async fn delete_refresh_token_by_auth_id<'e>(executor: impl sqlx::PgExecutor<'e>, user_auth_id: i32) -> Result<RefreshToken, MyError> {
    let mut query = sqlx::query_as(
        r#"
        DELETE FROM "refresh_tokens" 
...
pub async fn get_user_info_by_token<'e>(executor: impl sqlx::PgExecutor<'e>, refresh_token: String) -> Result<User, MyError> {
    let mut query = sqlx::query_as(
        r#"
        SELECT (us.id, us.last_name, us.first_name, us.type_user, us.profile_picture, ua.email)
